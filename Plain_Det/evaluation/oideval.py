...
class OIDEval:
    def __init__(
        ...
        # oid_hierarchy_path='/storage/data/zhuyuchen530/oid/annotations/challenge-2019-label500-hierarchy.json'):
        oid_hierarchy_path = '/inspurfs/group/yangsb/zhuyuchen/datasets/OID_V4/annotations/bbox_labels_600_hierarchy.json'):
        ...
        if isinstance(lvis_dt, LVISResults):
            ...
            # self.lvis_dt = LVISResults(self.lvis_gt, lvis_dt, max_dets=600)
        else:
    ...
    def _prepare(self):
        ...
        cat_per_img = defaultdict(list)
        for gt in gts:
            self._gts[gt["image_id"], gt["category_id"]].append(gt)
            cat_per_img[gt["image_id"]].append(gt['category_id']) 
        ...
        # img_gt = {d["id"]: d[""] for d in img_data}
        ...
        # print(f"_gts:-->{cat_per_img}")
        for dt in tqdm(dts):
            img_id, cat_id = dt["image_id"], dt["category_id"]
            # if cat_id not in cat_per_img[img_id]:
            #     continue
            if cat_id not in img_nl[img_id] and cat_id not in img_pl[img_id] and cat_id not in cat_per_img[img_id]:
                continue
            self._dts[img_id, cat_id].append(dt)
        print(f"{len(self._dts)}/{len(dts)}")
...
def _evaluate_predictions_on_oid(oid_gt, oid_results_path, eval_seg=False):
    ...
    if eval_seg:
        ...
        results["AP50"] = results["AP50_expand"]

...
class DeformableDETR(nn.Module):
    ...
    def __init__(
        ...
        self.class_embed_0 = nn.Linear(embed_dim, num_classes[0], bias=False)
        self.class_embed_1 = nn.Linear(embed_dim, num_classes[1], bias=False)
        self.class_embed_2 = nn.Linear(embed_dim, num_classes[2], bias=False)
        self.class_embed_3 = nn.Linear(embed_dim, num_classes[3], bias=False)
        ...
        if as_two_stage:
            ...
            # bias_value=0
            self.transformer.decoder.class_embed_0 = nn.Linear(embed_dim, num_classes[0])
            self.transformer.decoder.class_embed_1 = nn.Linear(embed_dim, num_classes[1])
            self.transformer.decoder.class_embed_2 = nn.Linear(embed_dim, num_classes[2])
            self.transformer.decoder.class_embed_3 = nn.Linear(embed_dim, num_classes[3])
    ...
    def forward(self, batched_inputs, dataset_name="lvis_v1"):
        ...
        inter_states_label = F.normalize(inter_states,p=2, dim=-1)
        ...
        if self.training:
            ...
            if self.count == 2000:
                ...
                if device == 0:
                    import json
                    with open('losslog/loss.json','w')as f:

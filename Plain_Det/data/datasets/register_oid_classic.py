...
def register_oid_instances_classic(name, metadata, json_file, image_root):
    """
    """
...
def load_coco_json_mem_efficient(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    ...
    for img_dict in imgs:
        record = {}
        if len(img_dict['file_name'].split('/')) > 1:
            record["file_name"] = os.path.join(image_root, 'train_'+img_dict["file_name"])
        else:

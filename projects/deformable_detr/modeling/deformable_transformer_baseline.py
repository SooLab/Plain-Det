...
class DeformableDetrTransformerEncoderB(TransformerLayerSequence):
    def __init__(
        self,
        embed_dim: int = 256,
        num_heads: int = 8,
        ...
        super(DeformableDetrTransformerEncoderB, self).__init__(
...
class DeformableDetrTransformerDecoderB(TransformerLayerSequence):
    def __init__(
        self,
        embed_dim: int = 256,
        num_heads: int = 8,
        ...
        super(DeformableDetrTransformerDecoderB, self).__init__(
            transformer_layers=BaseTransformerLayer(
                attn=[
                    MultiheadAttention(
                        embed_dim=embed_dim,
...
class DeformableDetrTransformerB(nn.Module):
    ...
    def __init__(
        ...
        super(DeformableDetrTransformerB, self).__init__()
        ...
        self.temp = nn.Linear(self.embed_dim, 512)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.as_two_stage:
            ...
            # output_memory_label  = torch.nn.functional.normalize(self.temp(output_memory) ,p=2, dim=-1)
            output_memory_label  = self.temp(output_memory)

            enc_outputs_class = self.decoder.class_embed[dataset_name][self.decoder.num_layers](output_memory_label)
            ...
            pos_trans_out = self.pos_trans_norm(
                # self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact, num_pos_feats=256))
                self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact))
            )
            query_pos, query = torch.split(pos_trans_out, c, dim=2)
            
            if self.learnt_init_query:
                query = query_embed #[None].repeat(bs, 1, 1)
                query_pos = None
        else:

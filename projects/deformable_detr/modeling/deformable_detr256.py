...
class DeformableDETR256(nn.Module):
    ...
    def __init__(
        ...
        # label embedding
        self.label_embedding = [torch.from_numpy(np.load(p)).\
            to(torch.float32).to(device) for p in label_embedding]
        ...
        for _ in range(num_label):
            num_basis = self.label_embedding[_].shape[0]
            label_in_channels = self.label_embedding[_].shape[1]
